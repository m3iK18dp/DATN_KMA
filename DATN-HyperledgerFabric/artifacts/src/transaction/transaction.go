...
func (t *TransactionContract) DeleteTransactionById(
	...
	transactionAsBytes, errC := ctx.GetStub().GetState(transactionId)
	if errC != nil {
		return "", fmt.Errorf("Failed to read from world state. %s", errC.Error())
	}
	if transactionAsBytes == nil {
		return "", fmt.Errorf("%s does not exist", transactionId)
	}
	// Registering the event
	var err = ctx.GetStub().SetEvent("DeleteTransEvent", transactionAsBytes)
	if err != nil {
		return "", fmt.Errorf("Failed to set event: %v", err)
	}
