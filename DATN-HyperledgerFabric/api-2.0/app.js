...
app.use(
  expressJWT({
    ...
    path: ["/users", "/users/login", "/register", "robots.txt", "websurge-allow.txt"],
  })
);
app.use(bearerToken());
...
app.use(robots(__dirname + "./websurge-allow.txt"));
logger.level = "debug";
...
async function main() {
  ...
  try {
    ...
    if (!identity) {
      ...
      await helper.enrollAdmin("Org1", await helper.getCCP("Org1"));
